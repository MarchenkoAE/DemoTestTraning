...
public static class Keyboard
{
    ...
    public static void Type(string text)
    {
        foreach (var c in text)
        {
            ...
            int vKeyValue = VkKeyScan(c);
            var keyIsShifted = (vKeyValue & VKeyShiftMask) == VKeyShiftMask;
            var key = KeyInterop.KeyFromVirtualKey(vKeyValue & VKeyCharMask);

            if (keyIsShifted) Type(key, new[] { Key.LeftShift });
            else Type(key);
        }
    }
    ...
    {
        foreach (var modiferKey in modifierKeys) Press(modiferKey);

        Type(key);

        foreach (var modifierKey in modifierKeys.Reverse()) Release(modifierKey);
    }
    ...
    private static void SendKeyboardInput(Key key, bool press)
    {
        ...
        {
            ki.union.keyboardInput.dwFlags |= KeyeventfExtendedkey;
        }
    ...
    private static readonly Key[] ExtendedKeys = new[] {
        Key.RightAlt,
        Key.RightCtrl,
        Key.NumLock,
        Key.Insert,
